...
export default function UserProfilePage() {
    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        if (data.success && data.user) {
          ...
          // ถ้าไม่มีข้อมูลใน DB ใช้ข้อมูลจาก localStorage
          setUser(localUser)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg({ type: "error", text: "Network error: " + error?.message })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {user && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-white/10 backdrop-blur-xl p-6 rounded-2xl border border-white/20 shadow-xl mb-6"
            >
            ...
            >
              ...
              <motion.div variants={itemVariants} className="flex items-center gap-6 mb-8">
                {preview ? (
                  ...
                  <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center ring-4 ring-white/20 shadow-lg">
                ...
                <label className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-white/10 hover:bg-white/20 border border-white/20 text-white cursor-pointer transition shadow-md hover:shadow-lg">
                  <ImageIcon className="w-5 h-5" />
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <motion.div variants={itemVariants} className="space-y-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setAllergies(e.target.value)}
                    rows={4}
                    className="w-full rounded-2xl px-4 py-3 bg-white/10 border border-white/20 text-white placeholder-blue-200/50 focus:outline-none focus:ring-2 focus:ring-blue-400/50 transition resize-none"
                  />
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="pt-4">
                  <motion.button
                    type="submit"
                    disabled={saving}
                    className="w-full inline-flex items-center justify-center gap-3 px-6 py-4 rounded-2xl font-bold text-lg text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl transition-all"
                    whileHover={{ scale: saving ? 1 : 1.02, y: saving ? 0 : -2 }}
                    whileTap={{ scale: 0.98 }}
                  >
      ...
      <style jsx global>{`
        ...
        ::-webkit-scrollbar-track { background: rgba(255,255,255,0.08); }
        ::-webkit-scrollbar-thumb {
          background: linear-gradient(to bottom, #3b82f6,rgb(92, 123, 246));
          border-radius: 4px;
        }
        ::-webkit-scrollbar-thumb:hover {
          background: linear-gradient(to bottom, #2563eb, rgb(92, 123, 246);
        }
      `}</style>
    </div>
  )
